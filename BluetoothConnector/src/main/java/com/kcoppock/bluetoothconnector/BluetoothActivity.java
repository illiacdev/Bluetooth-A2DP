...
import com.kcoppock.bluetoothconnector.databinding.ActivityMainBinding;
 ...
 */
public class BluetoothActivity extends FragmentActivity implements BluetoothBroadcastReceiver.Callback, BluetoothA2DPRequester.Callback {
...
//        setContentView(R.layout.activity_main);
        ...
        binding.selectDevice.setOnClickListener(v -> {
            FragmentManager manager = getSupportFragmentManager();
            DeviceSelectorFrgment frgment = new DeviceSelectorFrgment();
            frgment.setInterComm(device -> {
                selected_device = device;
                Log.i(TAG, "onSelected : " + device.getName());
                new BluetoothA2DPRequester(this).request(this, mAdapter);
            });
            frgment.show(manager, "test_1");
        });

        binding.connetDevice.setOnClickListener(v -> {
            Method disconnect = getDisConnectMethod();
            try {
                disconnect.setAccessible(true);
                disconnect.invoke(proxy, selected_device);
                binding.connetDevice.setVisibility(View.INVISIBLE);
        ...
        binding.connetDevice.setText("디바이스 해제");
        binding.connetDevice.setVisibility(View.INVISIBLE);
        ...
        binding.play.setOnClickListener(v -> {
            if (mPlayer != null) {
                mPlayer.stop();
                mPlayer = null;
                ((Button) v).setText("PLAY SOUND");
            } else {
                mPlayer = MediaPlayer.create(BluetoothActivity.this, R.raw.siren);
                mPlayer.setOnCompletionListener(mp ->
                                                {
                                                    mPlayer = null;
                                                    binding.play.setText("PLAY SOUND");
                                                }
                );
                mPlayer.start();
                ((Button) v).setText("STOP SOUND");
            }
        });
...
//        BluetoothDevice device = findBondedDeviceByName(mAdapter, HTC_MEDIA);
        BluetoothDevice device = findBondedDeviceByName(mAdapter, selected_device.getName());
        ...
        try {
            ...
            Log.i(TAG, "onA2DPProxyReceived: 연결됨");
            binding.connetDevice.setVisibility(View.VISIBLE);
...
//        connect.setAccessible(false);
    ...
    private Method getCloseMethod() {
        try {
            return BluetoothA2dp.class.getDeclaredMethod("close", BluetoothDevice.class);
        } catch (NoSuchMethodException ex) {
            Log.e(TAG, "Unable to find close(BluetoothDevice) method in BluetoothA2dp proxy.");
            return null;
        }
    }
    ...
    private static BluetoothDevice findBondedDeviceByName(BluetoothAdapter adapter, String name) {
        for (BluetoothDevice device : getBondedDevices(adapter)) {
            if (name.matches(device.getName())) {
                Log.v(TAG,
                      String.format("Found device with name %s and address %s.", device.getName(),
                                    device.getAddress()));
                return device;
            }
        }
